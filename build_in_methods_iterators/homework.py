...
def task_7_max_value_list_of_lists(data: List[List[int]]) -> int:
    ...
    """
    new_data = list(filter(lambda x: x, data))
    return max(max(new_data, key=lambda x: min(x)))
...
def task_11_create_list_of_random_characters() -> List[str]:
    ...
    """
    return [string.ascii_lowercase[random.randint(0, len(string.ascii_lowercase) - 1)] for _ in range(0, 20)]
